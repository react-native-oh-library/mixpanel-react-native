...
export default class MixpanelMain {
    ...
    getMetaData() {
        let metadata = {
            $os: deviceInfo.distributionOSName,
            $os_version: deviceInfo.distributionOSVersion,
    ...
    hasOptedOutTracking(token) {
        return this.mixpanelPersistent.getOptedOut(token);
    }
    ...
    async trackCharge(token, charge, properties) {
        ...
        await this.append(token, {
            $transactions: {
                $amount: charge, $time: Date.now(), ...properties
            },
        });
    }
    ...
    async trackWithGroups(token, eventName, properties, groups) {
        ...
        await this.track(token, eventName, {
            ...properties, ...groups
        });
    }
